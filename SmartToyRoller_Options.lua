...
RefreshPanel = function()
	...
	if profile and type(profile.groups) == "table" then
		for index, rowGroup in ipairs(profile.groups) do
			...
			row:ClearAllPoints()
			row:SetPoint("TOPLEFT", GROUP_X + 10 + ((index - 1) * 72), -246)
			row:SetSize(68, 24)
			row:SetText((index == selectedGroupIndex and "* " or "") .. rowGroup.label)
			row:SetNormalFontObject(index == selectedGroupIndex and "GameFontHighlightSmall" or "GameFontNormalSmall")
...
local function CreateOptionsPanel()
	...
	CreateBorderBox(panel, 216, -214, 378, 96)
	...
	panel.addGroup = CreateButton(panel, "新建分组", GROUP_X + 282, -220, 76)
	panel.addGroup:SetScript("OnClick", AddGroup)
	CreateLabel(panel, "当前分组操作", GROUP_X + 10, -278)
	panel.deleteGroup = CreateButton(panel, "删除分组", GROUP_X + 116, -276, 72)
	panel.deleteGroup:SetScript("OnClick", DeleteGroup)
	panel.groupUp = CreateButton(panel, "上移", GROUP_X + 192, -276, 48)
	panel.groupUp:SetScript("OnClick", function()
		MoveGroup(-1)
	end)
	panel.groupDown = CreateButton(panel, "下移", GROUP_X + 244, -276, 48)
